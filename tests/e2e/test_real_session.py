...
def _git_init(workspace: Path) -> None:
    ...
    subprocess.run(
        ["git", "commit", "-q", "-m", "seed"], cwd=workspace, check=True
    )
...
def _make_loop(
    ...
    *,
    ...
    manager = SandboxManager(workspace_root=workspace, preferred_tier=SANDBOX_TIER_SCOPED)
    tools: dict[str, Any] = {
...
def test_e2e_session_writes_file_and_runs_command(tmp_path: Path) -> None:
    ...
    assert any(m.role == "user" and "start the session" in m.content for m in session.messages)
...
def test_e2e_session_respects_safety_tier_dry_run(tmp_path: Path) -> None:
    ...
    async def driver() -> str:
        loop = _make_loop(
            workspace, db_path, provider, ui=_make_ui(approval_yes=False)
        )
        await loop.start_session()
...
def test_e2e_session_persists_through_db_restore(tmp_path: Path) -> None:
    ...
    provider2 = SessionProvider(
        scripts=[["<response>second loop</response>"]]
    )
