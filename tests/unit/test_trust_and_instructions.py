...
def test_workspace_trust_read_rejects_parent_swapped_after_validation(
    ...
    monkeypatch.setenv("HOME", str(home))
    real_open = trust_module.AnchoredDirectory.open
    swapped = False
    ...
    monkeypatch.setattr(
        trust_module.AnchoredDirectory,
        "open",
...
def test_workspace_trust_read_rejects_plain_parent_directory_swap(
    tmp_path, monkeypatch
) -> None:
    import ash.safety.trust as trust_module
    ...
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    (replacement_dir / "trusted-workspaces.json").write_text(
        json.dumps(
            {"version": 1, "workspaces": [str(workspace.resolve())]}
        )
        + "\n",
    ...
    assert is_workspace_trusted(workspace) is False
    assert swapped is True
    assert not (saved_dir / "trusted-workspaces.json").exists()
...
def test_workspace_trust_write_rejects_plain_parent_directory_swap(
    tmp_path, monkeypatch
) -> None:
    import ash.safety.trust as trust_module
    ...
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    victim = replacement_dir / "trusted-workspaces.json"
    victim.write_text(
        json.dumps({"version": 1, "workspaces": ["victim"]}) + "\n",
        encoding="utf-8",
    )
    monkeypatch.setenv("HOME", str(home))
    real_open = trust_module.AnchoredDirectory.open
    ...
    assert swapped is True
    assert (state_dir / "trusted-workspaces.json").read_text(encoding="utf-8") == (
        json.dumps({"version": 1, "workspaces": ["victim"]}) + "\n"
    )
    assert not (saved_dir / "trusted-workspaces.json").exists()
