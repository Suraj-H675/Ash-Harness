...
2. Create ash/tools/agent.py with ALL the following imports and code:
   ...
   class SpawnAgentTool(BaseTool):
       ...
       args_schema = type("SpawnAgentArgs", (BaseModel,), {"role": (str,...), "task": (str,...), "agent_id": (str | None, None)})
...
```
/goal phase8
Read /home/suraj/ash/PRIORITY.md. Implement Phase 8 ONLY in order: H-11 (memory recall) then M-13, M-14, M-15 then L-10, L-11. Do each one using its /goal command above. Do NOT implement any other item.
```

```
/goal phase9
Read /home/suraj/ash/PRIORITY.md. Implement Phase 9 ONLY in order: M-9 (retry backoff) then M-10 (circuit breaker suggestions) then L-16 (auto-approve). Do each one using its /goal command above. Do NOT implement any other item.
```

```
/goal phase10
Read /home/suraj/ash/PRIORITY.md. Implement Phase 10 ONLY in order: M-6 (TurnContext) then L-7 (token meter) then L-6 (RepoMap graph). Do each one using its /goal command above. Do NOT implement any other item.
```
...
```
/goal high
Read /home/suraj/ash/PRIORITY.md. Implement all HIGH priority items in order: H-1, H-2, H-3, H-4, H-5, H-6, H-7, H-8, H-9, H-10, H-11. Use each item's /goal command above. Do NOT implement MEDIUM or LOW items.
```

```
/goal medium
Read /home/suraj/ash/PRIORITY.md. Implement all MEDIUM priority items in order: M-1 through M-16. Use each item's /goal command above. Do NOT implement HIGH or LOW items.
```
...
```
/goal all
Read /home/suraj/ash/PRIORITY.md. Implement everything: run Phase 1 through Phase 12 in order using the phase commands above. Implements all 44 items.
```
