...
from tools.web import _normalize_allowed_domains, _validate_public_url
...
def _validate_browser_url(url: str, allowed_domains: tuple[str, ...]) -> str:
    ...
    if parsed.scheme in {"ws", "wss"}:
        equivalent = parsed._replace(scheme="https" if parsed.scheme == "wss" else "http")
        _validate_public_url(
            urlunparse(equivalent), allowed_domains=allowed_domains
        )
        return url
...
class BrowserScrollTool(_BrowserTool):
    name = "browser_scroll"
    description = "Scroll the current browser page up or down and return the updated snapshot."
