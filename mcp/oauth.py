...
async def authorize_mcp_server(
    ...
    *,
    ...
    try:
        ...
        announce(f"Open this URL to authorize MCP server {server_name!r}:\n{authorization_url}")
        opened = False
        try:
    ...
    for url in authorization_metadata_urls(issuer_hint):
        try:
            ...
            if not isinstance(methods, list) or "S256" not in methods:
                raise MCPOAuthError(
                    "authorization server does not advertise S256 PKCE"
                )
...
def _validate_oauth_url(value: str, *, allow_loopback_http: bool = False) -> str:
    ...
    if parsed.username or parsed.password or parsed.fragment:
        raise MCPOAuthError("OAuth endpoint URLs cannot contain credentials or fragments")
