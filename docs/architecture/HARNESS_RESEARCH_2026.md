...
| MCP | Strong partial | MCP 2026-07-28 stateless discovery/request envelopes, HTTP routing headers, required `resultType`, bounded MRTR for roots/sampling/elicitation, and in-band modern HTTP errors now coexist with the mature 2025-11-25 stateful path: strict envelopes, bounded stdio, version-aware schemas/content, restart-safe pagination, list-change refresh, session recovery, legacy tasks, GET/SSE, OAuth 2.1 refresh/scope step-up, and server requests. Modern `subscriptions/listen`, the redesigned `io.modelcontextprotocol/tasks` extension, and live multi-vendor conformance remain. |
