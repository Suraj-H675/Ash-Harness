...
- [Model Context Protocol 2026-07-28](https://blog.modelcontextprotocol.io/posts/2026-07-28/): stateless request envelopes, `server/discover`, HTTP routing headers, required `resultType`, bounded multi-round-trip requests, cacheable list results, subscriptions, and the extensions framework.
- [Model Context Protocol 2025-11-25](https://modelcontextprotocol.io/specification/2025-11-25): legacy stateful lifecycle and capability negotiation; JSON Schema 2020-12 tools; roots, sampling, elicitation, progress, cancellation, logging, authorization, and experimental core tasks.
...
| MCP | Strong partial | MCP 2026-07-28 stateless discovery/request envelopes, HTTP routing headers, required `resultType`, bounded MRTR, and automatic correlated `subscriptions/listen` streams for advertised tool/prompt/resource list changes now coexist with the mature 2025-11-25 stateful path: strict envelopes, bounded stdio, version-aware schemas/content, restart-safe pagination, atomic list-change refresh, session recovery, legacy tasks, GET/SSE, OAuth 2.1 refresh/scope step-up, and server requests. Per-resource `resourceSubscriptions`, the redesigned `io.modelcontextprotocol/tasks` extension, and live multi-vendor conformance remain. |
...
| Browser/media | Partial | Optional Playwright/Chromium navigation, ARIA snapshots, stable refs, form/click/scroll/history actions, network policy, screenshots/vision, bounded uploads, atomic bounded workspace downloads, private profiles, deterministic cleanup, and loopback runtime CDP connect/disconnect/status with isolated-context ownership are live. Remote CDP, direct takeover of existing tabs, and other media remain. |
...
1. Web search and an isolated browser automation baseline are live; richer
   ...
   workspace downloads and loopback runtime CDP attachment are live, while
   remote CDP/direct existing-tab control and other media remain.
