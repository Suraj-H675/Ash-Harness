...
| Security tests | Partial | Command bypass coverage includes shell-expanded destructive options, dynamic executables, common process/shell wrappers, and descriptor-anchored POSIX cwd race regressions across foreground/background command and sandbox-manager execution; broader path bypass coverage for other subprocess launchers, sandbox escape assumptions, and secret-leak coverage remain |
