...
| API-key providers | Partial | Anthropic, OpenAI, Groq, DeepSeek and custom endpoints tested from fresh process; custom OpenAI-compatible routes fail closed unless exact per-model metadata is explicitly declared; OpenRouter uses live per-model catalog metadata, and Mistral now starts conservative then enables tools/vision/context only from the selected model's provider-owned catalog entry; xAI/Together/Fireworks/Cerebras capability attribution remains a provider-specific audit item; runtime model switches close the previous provider deterministically |
...
| Model capability negotiation | Verified locally | Tools, vision, reasoning, local status, and known context/output limits use provider/model evidence rather than wire-protocol identity; Ollama, OpenRouter, Mistral, LM Studio, vLLM, and generic OpenAI-compatible routes can negotiate bounded live metadata before use, exact IDs and unique provider aliases resolve consistently while missing/malformed/ambiguous/different-model evidence stays conservative, and `/capabilities` distinguishes dynamic evidence even inside failover chains |
