...
| Security tests | Partial | Command bypass coverage includes shell-expanded destructive options, dynamic executables, common process/shell wrappers, descriptor-anchored POSIX cwd race regressions across foreground/background command, sandbox-manager, direct Git, patch, managed worktree Git, LSP, MCP stdio, durable automation, and command-hook execution; Linux Bubblewrap workspace and configured extra read-only mounts are descriptor-bound across foreground, background, and executable-plugin launches so post-validation pathname swaps cannot replace mounted content; descriptor-scoped custom-command reads also reject post-validation link swaps; broader path bypass coverage for other filesystem/subprocess surfaces, non-Bubblewrap sandbox mount races, sandbox escape assumptions, and secret-leak coverage remain |
