...
| IDE/ACP integration | Verified locally | Official ACP Python SDK v1 over bounded JSONL stdio; initialize/new/load/list/prompt/cancel, durable message and redacted tool replay, permission requests, text/resource links, ordered bounded inline image prompts, tool/usage streaming, editor-supplied stdio/HTTP/SSE MCP, isolated runtimes, and official-client plus shipped-process wire tests. ACP images are validated through Ash's canonical PNG/JPEG/GIF/WebP contract, limited to 5 MiB each / 10 MiB per prompt, passed as native provider image blocks in protocol order, rejected before provider I/O when the negotiated model lacks vision, and stripped of raw bytes before session persistence. Audio/embedded context, extra directories, modes, fork/resume, terminal/filesystem callbacks, and registry publication remain unadvertised. |
