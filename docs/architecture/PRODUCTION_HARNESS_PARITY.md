...
| Security tests | Partial | Command bypass coverage includes shell-expanded destructive options, dynamic executables, common process/shell wrappers, descriptor-anchored POSIX cwd race regressions across foreground/background command, sandbox-manager, direct Git, patch, managed worktree Git, LSP, MCP stdio, durable automation, command-hook execution, and direct executable-plugin launch; Linux Bubblewrap workspace and configured extra read-only mounts are descriptor-bound across foreground, background, and executable-plugin launches so post-validation pathname swaps cannot replace mounted content; on platforms with descriptor-anchored plugin snapshots, Docker executable-plugin code is captured from the discovered plugin inode and streamed into an Ash-owned daemon volume mounted read-only at runtime, eliminating the live host bind for that code path; descriptor-scoped custom-command reads also reject post-validation link swaps; broader path bypass coverage for other filesystem/subprocess surfaces, generic Docker workspace bind races, Docker plugin staging on platforms without anchored snapshots, sandbox escape assumptions, and secret-leak coverage remain |
