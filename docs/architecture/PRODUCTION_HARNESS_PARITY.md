...
| Fine-grained policy engine | Partial | Deny-first then ask/allow precedence, mode circuit breakers, stable scoped rules, conservative command-prefix parsing, managed policy layers, tool-specific path/domain grammar, and safe case-insensitive filename-extension matching are enforced; richer argument grammars remain |
