...
| Fine-grained policy engine | Partial | Deny-first then ask/allow precedence, mode circuit breakers, stable scoped rules, conservative command-prefix parsing, managed policy layers, and tool-specific path/domain grammar are enforced; richer argument grammars remain |
