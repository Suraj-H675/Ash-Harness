...
| API-key providers | Partial | Anthropic, OpenAI, Groq, DeepSeek and custom endpoints tested from fresh process; custom OpenAI-compatible routes fail closed unless exact per-model tool/vision/reasoning and limit metadata is explicitly declared; OpenRouter uses live per-model catalog metadata for tool, vision, reasoning, context, and output capability negotiation; runtime model switches close the previous provider deterministically |
