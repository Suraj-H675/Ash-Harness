...
from safety.scoped_io import ScopedIOError, read_scoped_bytes
...
class LSPClient:
        ...
        *,
        ...
        try:
            ...
            with contextlib.suppress(
                LSPError, OSError, asyncio.TimeoutError
            ):
                await self.notify(
                    "$/cancelRequest", {"id": request_id}, timeout=1.0
                )
            raise
        finally:
    ...
    async def _close(self) -> None:
        ...
        if process is None:
            self._closed = True
            return
        if process.returncode is None and self._reader_task is not asyncio.current_task():
            try:
                if self._open_close_enabled():
                    async with self._document_lock:
    ...
    async def _write_message(self, payload: dict[str, Any]) -> None:
        ...
        async with self._write_lock:
            try:
                ...
                raise LSPError(f"LSP server {self.config.name} closed its stdin") from exc
    ...
    def _sync_capability(self) -> int | dict[str, Any] | None:
        ...
        return value if isinstance(value, int | dict) and not isinstance(value, bool) else None
