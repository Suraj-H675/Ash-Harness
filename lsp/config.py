...
def _read_config(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(servers, dict) or len(servers) > MAX_LSP_SERVERS:
        raise ValueError(f"LSP config {path} servers must be an object of at most 32 entries")
    return servers
...
def _parse_server(
    ...
    if disabled:
        return replace(
            existing
            or LSPServerConfig(name, ("disabled",), {}, source=str(path)),
            disabled=True,
            source=str(path),
        )
    ...
    if not isinstance(env, dict) or len(env) > MAX_LSP_VALUES or not all(
        isinstance(key, str)
...
def _validate_string_list(
    ...
    if (
        ...
        raise ValueError(
            f"{label} must contain {minimum}..{maximum} bounded strings"
        )
...
def _resolve_executable(
    ...
    if allow_workspace and local.is_file() and (
        os.name == "nt" or os.access(local, os.X_OK)
    ):
