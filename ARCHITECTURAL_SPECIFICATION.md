...
from ash.core.context import ContextBuilder
...
class AshLoop:
    ...
    async def start_session(self, session_id: Optional[str] = None) -> None:
        """Initializes or restores a session by ID. If none provided, creates new."""
        ...

    async def run_turn(self, user_input: str) -> None:
        """Runs a single user-interaction turn until text response is completed."""
        ...
...
class CircuitBreaker:
    ...
    def record_failure(self, tool_name: str) -> None:
        """Increments failure count if the same tool fails consecutively."""
        ...
        if self.failure_counter >= self.max_failures:
            raise CircuitBreakerError(
                f"Tool '{tool_name}' failed {self.max_failures} times consecutively. "
                "Halting for user intervention."
            )

    def record_success(self) -> None:
        """Resets the circuit breaker metrics upon successful tool execution."""
        self.failure_counter = 0
...
class AshConfig(BaseSettings):
    ...
    provider: str = Field("anthropic", description="Primary model provider: 'anthropic' or 'openai' or 'ollama'")
    ...
    safety_tier: str = Field("interactive", description="Safety enforcement mode: 'interactive' or 'auto_approve' or 'dry_run'")
...
### 3.3 Subagent Shared State Schema (`shared_state.db`)
...
CREATE TABLE IF NOT EXISTS agent_status (
    ...
    status TEXT CHECK(status IN ('idle', 'working', 'failed', 'completed')),
    current_task TEXT,
...
CREATE TABLE IF NOT EXISTS sprints (
    ...
    state TEXT CHECK(state IN ('planning', 'active', 'complete', 'aborted')),
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
