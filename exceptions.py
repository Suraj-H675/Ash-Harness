...
def classify_exception(exc: BaseException) -> ErrorInfo:
    ...
    if _is_pydantic_validation_error(exc) or module == "config":
        return ErrorInfo(
    ...
    if name == "SandboxBackendUnavailable" or module.startswith("sandbox."):
        return ErrorInfo(
    ...
    if module.startswith("tools.") or name.endswith(("ToolError", "SkillParseError")):
        return ErrorInfo(
            ErrorCategory.TOOL,
            message,
    ...
    if module.startswith("context.") or "context" in lowered or "token" in lowered:
        return ErrorInfo(
